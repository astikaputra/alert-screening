...
class M_identifikasi extends CI_Model {
 ...
 function ambilDataIdentifikasiJoin(){
	    $this->db->select('tb_identifikasi.id_identifikasi, tb_identifikasi.tanggal, tb_identifikasi.identifikasi, tb_identifikasi.status, tb_identifikasi.oleh, tb_identifikasi.persentase, tb_permintaan_kartu.dari, tb_permintaan_kartu.departemen');
 			$this->db->where('tb_identifikasi.status','On Progress');
			$this->db->from('tb_identifikasi');
	    $this->db->join('tb_permintaan_kartu', 'tb_permintaan_kartu.id_permintaan = tb_identifikasi.id_identifikasi');
	    $query = $this->db->get();
	    if($query->num_rows()>0)
	    {
	      return $query->result();
 ...
 //    }
	...
	function simpanDataIdentifikasi(){
		$id = $this->input->post('txt_id_penerimaan');
